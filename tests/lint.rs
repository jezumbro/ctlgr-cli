...
#[test]
fn lint_does_not_process_md_files() {
    ...
    // lint on an .md file should succeed (no violations, .md skipped)
    cmd.args(["lint", "--file", &md_str]).assert().success();
...
#[test]
fn lint_write_does_not_convert_md() {
    let tmp = TempDir::new().unwrap();
    let md = tmp.path().join("page.md");
    std::fs::write(&md, "# Hello\n\nWorld.").unwrap();
    let md_str = md.to_string_lossy().to_string();
    let mut cmd = Command::cargo_bin("ctlgr").unwrap();
    cmd.args(["lint", "--write", "--file", &md_str]).assert().success();
    assert!(md.exists(), ".md must remain after lint --write");
    assert!(!tmp.path().join("page.html").exists(), ".html must not be created");
}

#[test]
fn lint_help_has_no_prefer_html() {
    let mut cmd = Command::cargo_bin("ctlgr").unwrap();
    let output = cmd.args(["lint", "--help"]).output().unwrap();
    let text = String::from_utf8_lossy(&output.stdout);
    assert!(!text.contains("prefer-html"), "prefer-html must not appear in lint --help");
}
